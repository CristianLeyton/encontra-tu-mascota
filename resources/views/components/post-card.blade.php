<div
    ...
    <div class="flex justify-between items-center text-sm text-gray-500 p-4">
        @if ($post->is_missing)
            <span class="flex items-center gap-1 px-2 py-1 rounded-md border border-red-600 text-red-600">
                Perdido
            </span>
        @else
            <span class="flex items-center gap-1 px-2 py-1 rounded-md border border-cyan-600 text-cyan-600">
                Encontrado
            </span>
        @endif
        <a href="{{ $post->location ? 'https://www.google.com/maps/search/' . urlencode($post->location) : null }}"
            class="inline-flex gap-1 px-2 py-1 rounded-md border border-lime-600 text-lime-600 hover:bg-lime-600 hover:text-white transition-colors max-w-[70%]"
            target="_blank">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="size-5 block">
    ...
    <div class="p-4 flex flex-col gap-1">
        ...
        @if (!$post->is_resolved)
            <div
                ...
                <span class="flex items-center gap-1 max-w-[99%]">
                    ...
                    <span class="text-left overflow-hidden whitespace-nowrap truncate max-w-[90%]">{{ $post->name_contact ?? 'Anónimo' }}</span>
                </span>
                ...
                @if ($post->email_contact)
                    <a href="mailto:{{ $post->email_contact }}" class="flex items-center gap-1.5 text-cyan-600 col-span-full"
                        target="_blank">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="size-6">
                            <path stroke-linecap="round" stroke-linejoin="round"
                ...
                @if ($post->phone_contact)
                    <a href="https://wa.me/{{ $post->phone_contact }}" class="flex items-center gap-1.5 text-lime-700 col-span-full"
                        target="_blank">
        ...
        @else
            <div
                ...
                ¡Caso resuelto!
                <svg xmlns="http://www.w3.org/2000/svg" class="size-7" width="24" height="24" viewBox="0 0 24 24"
            ...
            @auth
                @if ($post->user_id == auth()->user()->id)
                    <a href="/admin/posts?tableAction=edit&tableActionRecord={{ $post->id }}"
                        class="mt-3 w-full justify-center bg-amber-500 text-white text-sm px-3 py-1.5 rounded-lg hover:bg-amber-600 transition inline-flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
