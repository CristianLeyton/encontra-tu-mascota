<div class="p-4 pt-22 container max-w-6xl mx-auto xl:px-0">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 items-center">
                        ...
                        @foreach ($post->images as $image)
                            <div class="swiper-slide relative">
                                <img src="{{ asset('storage_public/' . $image) }}" alt="Foto de {{ $post->title }}"
                                    class="object-contain w-full aspect-video" lazy="true" loading="lazy">
        ...
        @endif
        <div
            class="bg-white rounded-xl shadow-md border border-gray-100 p-5 space-y-3 {{ empty($post->images) ? 'col-span-full' : 'col-span-1' }} ">
            {{-- Título y estado --}}
            <div class="text-center">
                ...
                <div class="flex justify-center gap-3">
                    ...
                    @if ($post->is_resolved)
                        <span
                            class="flex gap-1 text-emerald-600 bg-emerald-50 hover:bg-emerald-200/50 rounded-full px-3 py-1.5 transition items-center cursor-default font-semibold">
            ...
            <div class="flex flex-wrap gap-2 items-center justify-center text-sm">
                <p
                    class="inline-flex w-fit gap-1 text-gray-600 bg-gray-50 hover:bg-gray-200/50 rounded-full px-3 py-1.5 transition items-center cursor-default font-semibold group duration-300">
                ...
                <a target="_blank"
                    ...
                    class="inline-flex w-fit gap-1 text-lime-600 bg-lime-50 hover:bg-lime-200/50 rounded-full px-3 py-1.5 transition items-center cursor-pointer font-semibold group duration-300">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="size-5 text-lime-600 shrink-0">
                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <p
                    class="inline-flex w-fit gap-1 text-amber-600 bg-amber-50 hover:bg-amber-200/50 rounded-full px-3 py-1.5 transition items-center cursor-default font-semibold group duration-300">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    ...
                    Especie:</span>
                    {{ $post->species->name ?? 'No especificado' }}
                </p>

                <p
                    class="inline-flex w-fit gap-1 text-yellow-600 bg-yellow-50 hover:bg-yellow-200/50 rounded-full px-3 py-1.5 transition items-center cursor-default font-semibold group duration-300">
                    <span class="font-semibold flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="size-5 text-yellow-600 ">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0 1 11.186 0Z" />
                        </svg>
                        Raza:</span>
                    {{ $post->breed->name ?? 'No especificado' }}
                </p>

                <p
                    class="inline-flex w-fit gap-1 text-sky-600 bg-sky-50 hover:bg-sky-200/50 rounded-full px-3 py-1.5 transition items-center cursor-default font-semibold group duration-300">
                    <span class="font-semibold flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            style="fill: currentColor;transform: ;msFilter:;" class="size-5 text-sky-600">
                            <path
                        ...
                        </svg> Color:</span> {{ $post->color }}
                </p>

                <p
                    class="inline-flex w-fit gap-1 text-indigo-600 bg-indigo-50 hover:bg-indigo-200/50 rounded-full px-3 py-1.5 transition items-center cursor-default font-semibold group duration-300">
                    <span class="font-semibold flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
    ...
    @if (!$post->is_resolved)
        ...
        <div class="grid gap-2 grid-cols-1 md:grid-cols-2">
            <div
                ...
                <span
                    class="flex gap-1 text-gray-600 bg-gray-50 hover:bg-gray-200/50 rounded-full px-3 py-1.5 transition items-center font-semibold text-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                ...
                @if ($post->email_contact)
                    <a href="mailto:{{ $post->email_contact }}"
                        class="flex gap-1 text-sky-600 bg-sky-50 hover:bg-sky-200/50 rounded-full px-3 py-1.5 transition items-center font-semibold text-sm"
                        target="_blank">
                ...
                @if ($post->phone_contact)
                    <a href="https://wa.me/{{ $post->phone_contact }}"
                        class="flex gap-1 text-green-600 bg-green-50 hover:bg-green-200/50 rounded-full px-3 py-1.5 transition items-center font-semibold text-sm"
                        target="_blank">
            ...
            <div
                ...
                <a href="https://wa.me/?text={{ urlencode('Mascota perdida: ' . $post->title . ' - ' . route('post', $post)) }}"
                    target="_blank"
                    class="p-2 bg-green-500 hover:bg-green-600 text-white rounded-full flex items-center gap-1 shadow-md"
                ...
                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('post', $post)) }}"
                    target="_blank"
                    class="p-2 bg-blue-600 hover:bg-blue-700 text-white rounded-full flex items-center gap-1 shadow-md"
                ...
                <a href="https://twitter.com/intent/tweet?text={{ urlencode($post->title) }}&url={{ urlencode(route('post', $post)) }}"
                    target="_blank"
                    class="p-3 bg-slate-800 hover:bg-slate-700 text-white rounded-full flex items-center gap-2 shadow-md"
                ...
                <div class="relative">
                    <button title="Copiar enlace" type="button" id="copy-link-button"
                        class="cursor-pointer p-2 bg-amber-500 hover:bg-amber-600 text-white rounded-full flex items-center gap-2 shadow-md">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            stroke-width="1.5" stroke="currentColor" class="size-6">
                            <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div
                class="flex col-span-full lg:col-span-1 items-center gap-3 bg-white rounded-xl shadow-md px-4 py-3 border border-gray-100 group cursor-default">

                <div class="p-2 rounded-lg bg-green-100 text-green-600 text-xl mt-1 group-hover:bg-emerald-200/80 transition duration-300">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
        ...
        @auth
            @if ($post->user_id == auth()->user()->id)
                <a href="/admin/posts?tableAction=edit&tableActionRecord={{ $post->id }}"
                    class="justify-center bg-emerald-500 text-white text-sm px-3 py-1.5 rounded-lg hover:-translate-y-0.5 transition inline-flex items-center gap-2 w-full sm:w-fit">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
        ...
        @if (Auth::guest() || (Auth::check() && $post->user_id != auth()->user()->id))
            ...
            @else
                <button type="button" wire:click="reportPost" wire:loading.attr="disabled"
                    class="cursor-pointer flex items-center gap-2 bg-rose-600 text-white rounded-lg px-3 py-1.5 hover:-translate-y-0.5 transition w-full sm:w-fit justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="size-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
